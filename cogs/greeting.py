...
class Greeting(commands.Cog):
    ...
    async def setWelcomeChannel(self, ctx, channel: discord.TextChannel= None, *, welcome_text= None):
        ...
        if channel:
            if get_guild:
                ...
                if not welcome_text:
                    welcome_text = "Hello, {member.mention}. Welcome to **{member.guild}**"
                data = {"guild_id": ctx.guild.id, "welcome": True, "welcome_channel": channel.id, "welcome_msg": welcome_text}
                tb.insert_one(data)
                await ctx.send(f"Greeting channel has been set to {channel}")
        else:
            if get_guild:
                data = {"welcome": True}
                tb.update_one({'guild_id': ctx.guild.id}, {'$set': data})
                await ctx.send(f"Greeting channel has been set")
            else:
                data = {"guild_id": ctx.guild.id, "welcome": True}
                tb.insert_one(data)
                await ctx.send(f"Greeting channel has been set")
    
    #### Remove Welcome Channel ####
    @commands.command('rmWelcomeChannel', 'remWelcomeChannel')
    @has_permissions(administrator= True, manage_roles= True, manage_messages= True)
    async def removeWelcomeChannel(self, ctx):
        ...
        get_guild = tb.find_one({"guild_id": ctx.guild.id})
        if get_guild:
            tb.update_one({"guild_id": ctx.guild.id},{"$set": {'welcome': False}})
            await ctx.reply('Welcome channel successfully removed.', mention_author= True)
        else:
            data = {"guild_id": ctx.guild.id, "welcome": False}
            await ctx.reply('Welcome channel successfully removed.', mention_author= True)
...
#		welcomeChannel = c_fibu.execute("select welcome_channel from guild_data where guild_id=?",(member.guild.id,)).fetchone()
        ...
        db = con_fibu["fibu"] #database
        tb = db["guild_data"] #table
        guild = tb.find_one({"guild_id": member.guild.id})
        ...
        try:
            ...
            if welcome:
                ...
                if not welcomeChannel:
                    ...
                    welcomeChannel = get(member.guild.channels, id= guild["welcome_channel"])
                    if not welcomeChannel:
                        pass
                    else:
    ...
    ## hello, sorry, ok, thank you message reply ##
    @commands.Cog.listener()
    async def on_message(self, msg):
        if msg.content.lower() in ["!fibu hello", "!fibu hi", "!fibu hola", "!fibu hlw", "!fibu hlo", "!fibu hey", "hey fibu", "hello fibu", "hola fibu", "hi fibu"]:
            ...
            await msg.reply(f"Hello {msg.author.mention}! How are you?", mention_author= True)
        ...
        if msg.content.lower() in ["!fibu sorry","!fibu sry", "!fibu forgive me", "!fibu forgive", "sorry fibu","sry fibu", "forgive me fibu"]:
            await msg.add_reaction("🙂")
            await msg.reply("Ok... I forgive you. But don't repeat it again!", mention_author= True)
